...
int dist_pos_esq(int *barId , int regiao , struct barras_viga *bPtr , int *barraPtr , struct viga * vPtr )
{
    ...
    double ancoragem       = 0;
    double raioGancho      = 0;
    double armaduraVao     = 0;
    double armaduraApoio   = 0;
    double momentoDivisao  = 0;  
    int barrasDistribuidas = 0;  
    int barrasDivisao      = 0;  
    int barrasApoio        = 0;
    ...
    double bitola         = vReg->Bt_pos          ;  // (mm) Bitola da seção 
    ...
    //Nas regiões extremas, calcular ancoragem com gancho e verificar se é suficiente 
    ...
    if ( larguraApoioE <= (ancoragem + vPtr->geo.cob) ) //Sem espaço para ancoragem -> utilizar garantia de 7cm de cobrimento
    {
    ...
    // Caso esse novo valor não seja suficiente -> bitola não pode ser utilizada
        if ( larguraApoioE <= (ancoragem + vPtr->geo.cob) ) return 1;
    }  
    ...
    bPtr[numBarra].tipoRegiao = 5                                                           ; 
    strcpy(bPtr[numBarra].id,"Arm divisao (esq) positiva"); //Identificador de arm
...
for (int divisao = 0 ; divisao < vPtr->numDiv-1 ; divisao++)
{
    ...
    if (barrasDivisao + barrasDistribuidas > barrasMaxima) barrasDivisao = barrasMaxima - barrasDistribuidas;
